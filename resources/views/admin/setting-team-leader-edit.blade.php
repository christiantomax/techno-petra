...
<head>
    ...
    <title>Techno | Edit Team Leader</title>
                                ...
                                <form>
                                    <div class="row">
                                        <div class="col-md-6">
                                            ...
                                            <input id="id" type="hidden" name='id' class="form-control" value='{{$data['collections']->id}}'>
                                            <input id="nrp" type="text" name='nrp' class="form-control" value='{{$data['collections']->nrp_leader}}'>
                                        </div>
                                        <div class="col-md-6">
                                            <label for="period" class="form-label">Period</label>
                                            <select id="period" class="form-select" name="period">
                                                @foreach ($data['periods'] as $collection )
                                                    <option value={{$collection->id}}
                                                        @if ($collection->id == $data['collections']->id_period)
                                                            {{"selected"}}
                                                        @endif
                                                        >
                                                        {{$collection->year}} -
                ...
                success:function(data){
                    if(data.status == 200){
                        swal({
                            ...
                            function(isConfirm) {
                            if (isConfirm) {
                                window.location.href = "{{ route('team.list') }}";
                            }
                        });
                    }else{
